...
- **模型管理页帮助入口**：模型管理卡片标题旁新增帮助图标，点击后经系统浏览器打开[模型下载与安装帮助页](https://gitcode.com/tabortao/VelociText/discussions/1)。中英文界面均已适配。
...
### 修复
- **转录结果重复循环输出**：修复使用 Qwen3-ASR 模型转录时偶尔连续输出大量重复内容的问题（如 "the the the the ..."、"然后然后然后..."）。根因：Qwen3-ASR 为自回归生成模型，在近似贪心解码（temperature≈0）下可能陷入重复循环并一直生成到单段 token 上限，而 sherpa-onnx 未提供重复惩罚参数。现于每个分段识别完成后进行后处理折叠：同一字词连续出现 ≥6 次截断为 3 次、结尾同一短语连续重复 ≥3 次截断为 2 次；自然口语中的正常重复（"对对对"、"哈哈哈"、"非常好非常好"、"very very very"）不受影响。SenseVoice / Paraformer 为非自回归模型，不会产生循环，输出不受影响。新增 8 个单元测试覆盖循环折叠逻辑，全量测试 20/20 通过。
