...
- **Global shortcut for screenshot OCR**: the screenshot OCR shortcut (default `Ctrl+Shift+O`, configurable in Settings) now works globally via `tauri-plugin-global-shortcut`, even when the application is minimized to the system tray. The shortcut is registered on app startup and triggers the full screenshot capture and region selection workflow.
